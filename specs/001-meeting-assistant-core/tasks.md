...
- [ ] T001 Nâng IndexedDB lên version 2 trong `extension/lib/db.js`: thêm store `audio_chunks`
...
- [ ] T002 Unit test migration + API chunk trong `tests/unit/db.test.js` (fake-indexeddb):
      round-trip chunk theo seq, xóa theo meetingId, mở db v1 rồi nâng cấp v2 không mất meetings
- [ ] T003 Cập nhật `deleteMeeting` trong `extension/lib/db.js` xóa đủ 3 store
...
- [ ] T004 [US1] FR-020: trong `extension/offscreen/offscreen.js`, lắng nghe
      `track.onended`/`stream.oninactive` của tab stream → gọi `stopRecording()` (tái dùng
      luồng dừng chuẩn, không nhánh mới)
- [ ] T005 [US1] E2E verify luồng lõi + đóng offscreen giữa chừng không vỡ viewer trong
...
- [ ] T006 [P] [US2] Tách hàm `labelSpeaker(rmsMic, rmsTab)` (ngưỡng 1.4×) từ
      `extension/offscreen/offscreen.js` về `extension/lib/segmenter.js`; offscreen import lại
- [ ] T007 [US2] Harness SC-008 trong `tests/unit/speaker-label.test.js`: fixture PCM tổng hợp
...
- [ ] T009 [US4] E2E trong `tests/e2e/extension.spec.js`: xóa meeting có chunk mồ côi →
...
- [ ] T010 [US5] Persist chunk dần trong `extension/offscreen/offscreen.js`:
...
- [ ] T015 [P] [US5] Demuxer `extension/lib/webm-opus.js` (thuần, ~200 dòng): parse EBML
      tối giản → yield `{opusPacket, timestampMs}` từ WebM do MediaRecorder Chrome sinh;
      không dependency ngoài
- [ ] T016 [P] [US5] Unit test `tests/unit/webm-opus.test.js` với fixture WebM nhỏ
...
- [ ] T017 [US5] Re-transcribe streaming trong `extension/offscreen/offscreen.js`:
...
- [ ] T018 [US5] Thanh tiến độ re-transcribe trong `extension/viewer/viewer.js|html`
      (consume `pipeline-status.progress` theo contract messages.md)

### Model UX (FR-018 / SC-005 — research R3)

- [ ] T019 [P] [US5] Lệnh `prepare-model` theo contracts/messages.md: popup → background →
      offscreen `transcriber.load(model)`; nút "Chuẩn bị model" + % tiến độ trong
      `extension/popup/popup.html|js` (consume `model-progress`)
- [ ] T020 [P] [US5] Hiển thị `model-progress` khi re-transcribe trong `extension/viewer/viewer.js`

### Quota (FR-019 — research R4)

- [ ] T021 [P] [US5] Module `extension/lib/storage-policy.js` (thuần):
...
- [ ] T022 [US5] Popup: quota bar + chặn "Bắt đầu ghi" khi `critical` kèm hướng dẫn dọn,
      trong `extension/popup/popup.js|html|css`; gọi `navigator.storage.persist()` một lần
- [ ] T023 [US5] Viewer: quota bar + dung lượng từng phiên trong
...
- [ ] T025 [P] Cập nhật `specs/001-meeting-assistant-core/contracts/messages.md` nếu contract
